#!/bin/bash
# End-to-end tests for the diun-traefik-letsencrypt-docker-compose
# backup + restore flow.
#
# Diun keeps one small database: the images it has already seen and the digests
# they resolved to. Losing it is not a disaster - the next scan rebuilds it -
# but with first-check notification on, rebuilding means one message per image
# you run, all at once. The test that matters is whether a file written before
# a backup comes back after a restore that has since destroyed it.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-diun}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-diun-docker-compose.yml}"
...
: "${DATA_BACKUPS_PATH:=/srv/diun-data/backups}"
: "${DATA_BACKUP_NAME:=diun-data-backup}"
: "${DATA_PATH:=/data}"
: "${BACKUP_INTERVAL:=24h}"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq diun | head -n 1)"
[[ -n "$BACKUPS_CONTAINER" ]] || { echo "error: backups container not found" >&2; exit 1; }
[[ -n "$APP_CONTAINER" ]] || { echo "error: diun container not found" >&2; exit 1; }
...
test_env_required() {
  # Diun has no web interface and no certificate, so there is exactly one
  # required variable: somewhere to send what it finds. A notifier with nowhere
  # to notify is the one configuration that makes the whole stack pointless
  # while looking entirely healthy.
  grep -qE '^DIUN_WEBHOOK_URL=.' .env || { echo "  DIUN_WEBHOOK_URL is not set in .env" >&2; return 1; }
...
test_backup_carries_the_database() {
  ...
  # diun.db is the only thing in here worth keeping.
  local f; f=$(list_backups | tail -1)
  bk "tar -tzf '$f' | grep -q '^data/diun.db'" \
    || { echo "  $f does not carry diun.db" >&2; return 1; }
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./diun-restore-data.sh "$(basename "$f")" > /dev/null \
    || { echo "  ./diun-restore-data.sh failed" >&2; return 1; }
