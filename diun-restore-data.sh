#!/usr/bin/env bash
# diun-restore-data.sh [backup-file-name]
#
# Replaces Diun's application data with one of the archives the backups
# service wrote.
#
#   ./diun-restore-data.sh               list and ask
#   ./diun-restore-data.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than diun.
set -Eeuo pipefail
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Diun." >&2
  exit 1
fi
